...
- **README: Migration guide (v2 → v3)** — fixes the "map not rendering after upgrade" class of issues: per-bundler `setWorkerUrl()` setup (Vite `?worker&url`, webpack 5/CRA `new URL(…, import.meta.url)`, Next.js both modes, esbuild/Rollup), TypeScript `module`/`target` requirements (`import.meta` needs ES2020+), CRA 5 + Jest `transformIgnorePatterns`, Next.js compiler/server-bundling notes (`transpilePackages` not needed; `serverExternalPackages` for server-pass require errors), and the engine v6 behavior-change checklist (named imports only, class events + `e.type`, `setMissingStyleImageResolver` replacing `styleimagemissing` addImage, nested GeoJSON properties, CSP directives). Next.js worker-fix section widened to both bundler modes; `predev`/`prebuild` hooks now recommended over `postinstall`.
