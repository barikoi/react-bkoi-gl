...
- **Unit coverage raised from 88% to 98%** (lines, full unit + browser run): MinimapControl lifecycle/sanitizer/responsive-sizing/map-sync, worker URL registration (override respected, Blob fallback, SSR no-op), Maplibre map reuse reparenting + `gl` context hijack + hover warning paths, layer event handler bodies, Geolocate StrictMode guards, Globe projection fallbacks, and guard-branch tests across controls, sources, and utils. README carries a CI status badge and a coverage badge (both temporarily scoped to the `maplibre-v6.6.0` branch until it merges — master has no workflow runs yet; both need the repo public to render off-GitHub, since GitHub badge SVGs and `raw.githubusercontent` never resolve for private repos) plus a registry-driven npm types badge.
