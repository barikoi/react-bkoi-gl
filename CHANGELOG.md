...
- **No CI — all gates local** (deliberate): typecheck, lint, full tests (unit + browser), e2e, and the pack gate run locally; `prepublishOnly` enforces typecheck + lint + tests + build before any publish. The README shows self-contained `coverage-badge.svg` and `tests-badge.svg` (committed, referenced by relative path — render on private repos, no external fetch, zero CI), regenerated automatically by `npm test` / `npm run coverage` via `scripts/make-badges.mjs`; coverage color bands match `barikoiapis-golang` (90/80/70/60). The remaining GitHub badges (npm version/types/downloads/node/license) are registry-driven and need no CI.
