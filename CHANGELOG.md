...
- **Minimal CI pipeline** (`.github/workflows/ci.yaml`, `barikoiapis-golang` pattern): one job, one Node version — typecheck, lint, unit tests (jsdom, no Playwright download). Browser tests, e2e, build, and the pack gate run locally and are enforced by `prepublishOnly` before publishing. The README shows a self-contained `coverage-badge.svg` (committed, referenced by relative path — renders on private repos, no external fetch), regenerated automatically by `npm run coverage`; color bands match `barikoiapis-golang` (90/80/70/60).
