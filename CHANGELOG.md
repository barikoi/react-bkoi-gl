...
- **Unit coverage raised from 88% to 98%** (lines, full unit + browser run): MinimapControl lifecycle/sanitizer/responsive-sizing/map-sync, worker URL registration (override respected, Blob fallback, SSR no-op), Maplibre map reuse reparenting + `gl` context hijack + hover warning paths, layer event handler bodies, Geolocate StrictMode guards, Globe projection fallbacks, and guard-branch tests across controls, sources, and utils. README now carries a live CI status badge and a coverage badge backed by the committed `coverage.json` endpoint.
