...
- **CI pipeline** (`.github/workflows/ci.yaml`): Node 20/22 matrix — typecheck, lint, build, tests with coverage. Coverage badge follows the `barikoiapis-golang` pattern: CI publishes `coverage.json` to a dedicated orphan `badges` branch (force-pushed from the test job itself, no duplicate run, no bot commits to the default branch); the README badge reads that branch and color bands match the Go repo (90/80/70/60).
