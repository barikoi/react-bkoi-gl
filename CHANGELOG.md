...
- **CI pipeline** (`.github/workflows/ci.yaml`): master-only Node 20/22 matrix — typecheck, lint, build, tests with coverage. No CI badge publishing (kept lean and cheap; coverage is validated locally). The README shows a self-contained `coverage-badge.svg` (committed, referenced by relative path — renders on private repos, no external fetch), regenerated automatically by `npm run coverage`; color bands match `barikoiapis-golang` (90/80/70/60).
