...
- **CI pipeline** (`.github/workflows/ci.yaml`): Node 20/22 matrix — typecheck, lint, build, tests with coverage. Coverage badge follows the `barikoiapis-golang` pattern: CI publishes `coverage.json` to a dedicated orphan `badges` branch (force-pushed from the test job itself, no duplicate run, no bot commits to the default branch); color bands match the Go repo (90/80/70/60). While the repo is private, the README instead shows a self-contained `coverage-badge.svg` (committed, referenced by relative path — renders on github.com without external fetches); the endpoint badge replaces it once the repo is public.
...
- **`npm run test:resolution`** (`scripts/test-resolution.mjs`) — package-manager resolution matrix without full framework builds: installs the packed tarball in a per-manager sandbox (npm, pnpm, yarn classic, yarn-berry PnP, bun) and imports `react-bkoi-gl` by name, resolving `.`, `./styles`, and `./worker` plus version parity. The yarn-berry cell runs under PnP's `.pnp.cjs` loader (Node's experimental ESM loader under PnP is a known engine gap, not a package defect).
- **`test:pack` gate extended** — before the install sandbox, `publint --strict` (packaging/lint hygiene) and `@arethetypeswrong/cli` (`attw --pack --entrypoints .`, verifying the dual CJS/ESM types of the type-bearing entrypoint across node10/node16-ESM/node16-CJS/bundler resolution) must pass.
...
- **Unit coverage raised from 88% to 98%** (lines, full unit + browser run): MinimapControl lifecycle/sanitizer/responsive-sizing/map-sync, worker URL registration (override respected, Blob fallback, SSR no-op), Maplibre map reuse reparenting + `gl` context hijack + hover warning paths, layer event handler bodies, Geolocate StrictMode guards, Globe projection fallbacks, and guard-branch tests across controls, sources, and utils. README carries a CI status badge (scoped to the development branch until it merges — master has no workflow runs yet) plus registry-driven npm/types badge.

### Fixed
- **`animate-camera` e2e timed out in headed runs** — maplibre v6's `isStyleLoaded()` folds tile completeness into "style loaded" (`Style#loaded` → `TileManager#loaded`: every in-view tile must be loaded/errored), and this case animates the camera continuously from `onLoad` (15°/s orbit), churning the in-view tile set every frame. On a maximized headed display (≈4× the tiles of headless) the fixture's settle gate could never sample true — verified causally: stopping the orbit flips `isStyleLoaded()` true within ~2s. Continuously-animating cases now gate on a parsed style instead.
- **Headed review UI unified** — every headed e2e surface (`playwright test --headed`, `e2e:review`, `test:framework:review`) now shows the exact same bottom-center HUD ("Rendering · <case>") with a draining 10s hold progress bar. Two gaps found: the spec-run hold was dead code — it detected headless via `project.use.headless`, which the CLI `--headed` flag never populates (fixtures now also read the runner's `process.argv`); and the `e2e:review` runner had never been wired to the shared HUD module — it still injected a duplicate top-left banner (now deleted) while the shared `reviewBannerScript` sat unused. Also fixed the runner's "not in review modules" warning, which compared case ids (`map/basic`) to module names (`map`) with `===` and so flagged all 39 cases.
- **Framework runner: verify phase starved its own server** — the static-serve + headless verification ran the browser probe through a blocking `execSync` on the same event loop that served the app, so Chromium's navigation timed out under load; the runner now spawns verification asynchronously (`shAsync`) while the server stays responsive. `pnpm` cells also no longer fail on blocked dependency build scripts (`ERR_PNPM_IGNORED_BUILDS` — pnpm ≥10 defaults, `strictDepBuilds` hard-fails in pnpm 11): the runner writes the manager's expected approval file (`allowBuilds` map for pnpm 11, legacy `onlyBuiltDependencies` list for v10 — never both; pnpm 11 rewrites the file to an inert placeholder when both are present) covering `esbuild`/`core-js` postinstalls. Switching package managers over an app directory now resets a foreign-layout `node_modules` (npm over a pnpm tree died on `EUNSUPPORTEDPROTOCOL workspace:*`).
