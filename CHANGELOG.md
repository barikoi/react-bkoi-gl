...
- **E2e suite expanded to 33 tests / 32 case pages** with open-data MapLibre-example cases (no Barikoi tiles required): hillshade (Terrarium DEM), clustered points, heatmap, animated marker, video source (CORS-open Wikimedia footage), fitBounds/pitch/bearing controls, globe with atmosphere (EOx satellite), 3D buildings + terrain (OpenFreeMap `planet` + Terrarium DEM). DrawControl coverage now spans the full README prop surface (`displayControlsDefault`, `defaultMode`, custom `styles`, `onDrawModeChange`, trash delete) alongside create/update/select/delete interactions. Headed review runs get a shared "Rendering ·" status pill, a 10s per-test hold, and maximized windows (no fixed viewport — never crops on any OS/display).
