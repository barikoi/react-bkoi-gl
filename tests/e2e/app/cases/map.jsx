  ...
  return (
    <Section title='Controlled viewState — click Move, camera jumps'>
      <div className='map-ui'>
        <button
          data-testid='move'
          onClick={() => setViewState({ ...viewState, longitude: 90.0, latitude: 23.0, zoom: 14 })}
        >
          Move
        </button>
      </div>
...
export function MapEventsExtended() {
  ...
  // The ticker makes the events VISIBLY fire during headed review.
  const [ticker, setTicker] = useState([])
  const logEvent = type => e => {
    window.__log({ type })
    setTicker(t => [...t.slice(-7), type])
  }
  return (
    <Section title='Events — drag / hover / zoom / resize / idle'>
      ...
      >
        {ticker.length === 0
          ...
          : ticker.map((t, i) => (
              <div key={i} style={{ opacity: 0.45 + (0.55 * (i + 1)) / ticker.length }}>
                ▸ {t}
              </div>
            ))}
      </div>
      <TestMap
        ...
        onDrag={logEvent('drag')}
        onDragEnd={logEvent('dragend')}
        onMouseEnter={logEvent('mouseenter')}
        onMouseLeave={logEvent('mouseleave')}
        onMoveStart={logEvent('movestart')}
        onZoomStart={logEvent('zoomstart')}
        onResize={logEvent('resize')}
        onIdle={logEvent('idle')}
      >
        <Source
          id='hover-src'
          type='geojson'
        ...
        >
          <Layer
            id='hover-target'
            type='circle'
            paint={{ 'circle-radius': 12, 'circle-color': '#f0f' }}
          />
        </Source>
      </TestMap>
    </Section>
