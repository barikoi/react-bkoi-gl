...
// One test per URL — a headed review must not re-render the same URL once
// per assertion group.
import {
  test,
  expect,
...
test('map/basic: style + initialViewState + single onLoad + canvas + branding', async ({
  page,
}) => {
  await gotoCase(page, 'map/basic')
  const sec = page

  // Real Barikoi style applied with initialViewState
  ...
  // Branding: logo + attribution render by default
  ...
  // Logo is visually painted, not just present in the DOM — regression guard
  // for the styles export (`react-bkoi-gl/styles`): the logo anchor has no
  // content; its entire visual is the background-image in dist/styles CSS.
  // If that CSS fails to load/resolve the logo disappears while existence
  // assertions above still pass.
  const logo = sec.locator('a.maplibregl-ctrl-logo[href*="barikoi.com"]')
  ...
  // Attribution copyright survives maplibre rebuilds when tiles land —
  // the Barikoi © links must be re-applied and stay visible.
  await expect
    .poll(
      async () =>
...
test('map/no-defaults: showAttribution=false hides attribution; logo always renders', async ({
  page,
}) => {
...
test('map/controlled: viewState props drive the camera', async ({ page }) => {
...
test('map/events: onClick lngLat; onMoveEnd/onZoomEnd fire on interaction', async ({ page }) => {
  ...
  expect(clicks[0].lngLat.lng).toBeLessThan(90.4)
...
test('map/ref-methods: MapRef methods expose the maplibre instance', async ({ page }) => {
...
test('map/events-extended: drag, hover, resize, idle, movestart/zoomstart', async ({ page }) => {
  ...
  // Resize: call map.resize() — setViewportSize collapses maximized headed
  // windows, and a synthetic window resize event is ignored by maplibre
  // (dimensions unchanged). map.resize() fires the resize event → onResize.
  await page.evaluate(() => window.__MAP__.resize())
  await expect
