...
describe('ScaleControl Component', () => {
  ...
  beforeEach(() => {
    ...
    mockMapLib = {
      ScaleControl: vi.fn().mockImplementation(function (options) {
        // Store the options for later verification. Clone: the real control
        // copies options into its own mutable object — assigning React's
        // (frozen) props object breaks later option updates.
  ...
  test('rerender with a changed maxWidth updates the existing control options', () => {
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <ScaleControl maxWidth={100} unit='metric' />
      </MapContext.Provider>
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <ScaleControl maxWidth={250} unit='metric' />
      </MapContext.Provider>
    )

    // the update effect ran on the SAME control instance
    expect(mockScaleControlInstance.options.maxWidth).toBe(250)
    expect(mockScaleControlInstance.setUnit).not.toHaveBeenCalled()
  })
})
