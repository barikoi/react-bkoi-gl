...
// Mock Maplibre class
// Wrapper instances created by the component under test (constructor-time
// access only — safe with vitest's hoisted mock factory)
...
describe('Map Component', () => {
  ...
  test('renders Map component with correct props', () => {
    const { container } = render(
      <Map id='test-map' style={{ width: '500px', height: '400px' }} center={[0, 0]} zoom={5} />
    )
  ...
  test('prop changes are forwarded to the map instance via setProps', async () => {
    const { rerender } = render(<Map id='props-map' zoom={10} center={[0, 0]} />)

    await waitFor(() => {
      expect(wrapperInstances.length).toBeGreaterThan(0)
    })
    const wrapper = wrapperInstances[wrapperInstances.length - 1]
    wrapper.setProps.mockClear()

    // changed prop → diff detects it
    rerender(<Map id='props-map' zoom={12} center={[0, 0]} />)
    await waitFor(() => {
      expect(wrapper.setProps).toHaveBeenCalled()
    })

    // removed prop → reverse diff detects it
    wrapper.setProps.mockClear()
    rerender(<Map id='props-map' center={[0, 0]} />)
    await waitFor(() => {
      expect(wrapper.setProps).toHaveBeenCalled()
    })
  })

  test('a default-export mapLib without Map is rejected', async () => {
    const originalConsoleError = console.error
    console.error = vi.fn()

    // legacy default-export shape whose default lacks Map
    const legacyPromise = Promise.resolve({ default: {} })

    act(() => {
      render(<Map id='legacy-maplib' mapLib={legacyPromise} />)
    })
