...
describe('GeolocateControl Component', () => {
  ...
  test('adds the control with correct props', () => {
    ...
    expect(useControlModule.useControl).toHaveBeenCalledWith(expect.any(Function), {
  ...
  test('registers event handlers correctly', () => {
    ...
    expect(mockGeolocateControlInstance.on).toHaveBeenCalledWith(
      'outofmaxbounds',
      expect.any(Function)
    )
    expect(mockGeolocateControlInstance.on).toHaveBeenCalledWith(
  ...
  test('supports multiple instances with different options', () => {
    ...
    mockMapLib.GeolocateControl.mockImplementationOnce(function () {
    ...
    expect(useControlModule.useControl).toHaveBeenNthCalledWith(1, expect.any(Function), {
      position: 'top-left',
    })
    expect(useControlModule.useControl).toHaveBeenNthCalledWith(2, expect.any(Function), {
      position: 'bottom-right',
    })
  })
  ...
  test('_setupUI wrapper skips re-initialization when the container already has children', () => {
    // Pre-populate the container to simulate a StrictMode double-mount where
    // the UI was already created
    mockGeolocateControlInstance._container.appendChild(document.createElement('div'))
    const setupUI = vi.fn()
    mockGeolocateControlInstance._setupUI = setupUI
    ...
    const wrappedSetupUI = mockGeolocateControlInstance._setupUI
    expect(wrappedSetupUI).not.toBe(setupUI)

    wrappedSetupUI()
    // guard holds: the inner setupUI must not run for a populated container
    expect(setupUI).not.toHaveBeenCalled()
  })

  test('empty container lets the wrapped _setupUI run the original', () => {
    const setupUI = vi.fn()
    mockGeolocateControlInstance._setupUI = setupUI
    ...
    mockGeolocateControlInstance._setupUI()
    expect(setupUI).toHaveBeenCalledTimes(1)
  })

  test('_finishSetupUI runs once per button element (StrictMode double-mount guard)', () => {
    const finishUI = vi.fn()
    mockGeolocateControlInstance._finishSetupUI = finishUI
    const button = document.createElement('button')
    mockGeolocateControlInstance._geolocateButton = button
    ...
    const wrapped = mockGeolocateControlInstance._finishSetupUI
    wrapped('a')
    // second queued invocation for the SAME button must not re-attach the
    // click listener
    wrapped('a')

    expect(finishUI).toHaveBeenCalledTimes(1)
    expect(finishUI).toHaveBeenCalledWith('a')
  })

  test('trackuserlocationstart/end events forward to onTrackUserLocationStart/End', () => {
    const onTrackUserLocationStart = vi.fn()
    const onTrackUserLocationEnd = vi.fn()
    ...
    const emit = event =>
      mockGeolocateControlInstance.on.mock.calls
...
describe('Direct _GeolocateControl function tests for branch coverage', () => {
  ...
  test('full conditional branch coverage for _GeolocateControl', () => {
    ...
    const trackStartCallback = mockGC.on.mock.calls.find(
      call => call[0] === 'trackuserlocationstart'
    )[1]
    const trackEndCallback = mockGC.on.mock.calls.find(
      call => call[0] === 'trackuserlocationend'
    )[1]
