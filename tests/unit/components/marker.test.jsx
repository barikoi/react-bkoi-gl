...
describe('Marker Component', () => {
  ...
  test('renders a marker with correct props', () => {
    ...
    expect(mockMapLib.Marker).toHaveBeenCalledWith(
  ...
  test('updates marker properties when props change', () => {
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={10} latitude={20} offset={[0, 0]} draggable={false} rotation={0}>
          <div>Marker content</div>
        </Marker>
      </MapContext.Provider>
    )
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={15} latitude={25} offset={[5, 5]} draggable={true} rotation={45}>
  ...
  test('applies custom style to marker element', () => {
    ...
    render(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={10} latitude={20} style={customStyle}>
    ...
    expect(applyReactStyle).toHaveBeenCalledWith(mockElement, customStyle)
  })
  ...
  test('handles marker click events', () => {
    ...
    render(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={10} latitude={20} onClick={onClickMock}>
  ...
  test('handles marker drag events', () => {
    ...
    const dragHandler = mockMarkerInstance.on.mock.calls.find(call => call[0] === 'drag')[1]

    const dragendHandler = mockMarkerInstance.on.mock.calls.find(call => call[0] === 'dragend')[1]
  ...
  test('updates popup when prop changes', () => {
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={10} latitude={20} popup={mockPopup}>
          <div>Marker content</div>
        </Marker>
      </MapContext.Provider>
    )
  ...
  test('toggles class names when className prop changes', () => {
    ...
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={10} latitude={20} className='old-class'>
          <div>Marker content</div>
        </Marker>
      </MapContext.Provider>
    )
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Marker longitude={10} latitude={20} className='new-class'>
          <div>Marker content</div>
        </Marker>
      </MapContext.Provider>
    )
  ...
  test('forwards ref to marker instance', () => {
    ...
    render(
      <MapContext.Provider value={mapContextValue}>
        <Marker ref={ref} longitude={10} latitude={20}>
          <div>Marker content</div>
        </Marker>
      </MapContext.Provider>
    )
