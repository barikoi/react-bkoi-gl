  ...
  console.error = vi.fn((...args) => {
    ...
    const isExpectedMapError =
      ...
      (firstArg &&
        ...
        ((firstArg.detail &&
          typeof firstArg.detail.message === 'string' &&
...
describe('MapProvider and useMap', () => {
  ...
  test('MapProvider initializes with empty maps collection', () => {
    ...
    const TestComponent = () => {
      ...
      return <div data-testid='maps-count'>{Object.keys(context.maps).length}</div>
    }
  ...
  test('onMapMount adds a map to the collection with specified ID', async () => {
    ...
    const TestComponent = () => {
      ...
      return <div data-testid='map-id'>{context.maps[uniqueId] ? 'Map added' : 'No map'}</div>
    }
  ...
  test('onMapMount adds a map with default ID when no ID is provided', async () => {
    ...
    const TestComponent = () => {
      ...
      return <div data-testid='map-id'>{context.maps['default'] ? 'Map added' : 'No map'}</div>
    }
  ...
  test('onMapUnmount does nothing when map with given ID does not exist', async () => {
    ...
    const TestNonExistentMapComponent = () => {
      ...
      return (
        <div data-testid='test-result'>{state.existingMapStillExists ? 'Success' : 'Failed'}</div>
      )
    }
  ...
  test('onMapUnmount removes a map with default ID when no ID is provided', async () => {
    ...
    const TestDefaultMapComponent = () => {
      ...
      return <div data-testid='map-removed'>{state.removed ? 'Removed' : 'Not removed'}</div>
    }
  ...
  test('useMap accesses maps from MountedMapsContext', async () => {
    ...
    const TestMountedMapsComponent = () => {
      ...
      return (
        <div data-testid='map-exists'>{mapAdded && maps[uniqueId] ? 'Map exists' : 'No map'}</div>
      )
    }
...
test('real MapProvider.onMapMount rejects the reserved id "current"', async () => {
  let onMapMount
  const Grab = () => {
    onMapMount = React.useContext(MountedMapsContext).onMapMount
    return null
  }
  ...
  // the updater runs during act's render flush, so the throw escapes the
  // callback — catch it around act() itself
  expect(() =>
    act(() => {
      onMapMount({ id: 'test-map' }, 'current')
    })
  ).toThrow("'current' cannot be used as map id")
})

test('real MapProvider.onMapMount rejects duplicate map ids', async () => {
  let onMapMount
  const Grab = () => {
    onMapMount = React.useContext(MountedMapsContext).onMapMount
    return null
  ...
  const uniqueId = createUniqueId('dup-real')
  act(() => {
    onMapMount({ id: 'test-map' }, uniqueId)
  })

  expect(() =>
    act(() => {
      onMapMount({ id: 'second' }, uniqueId)
    })
  ).toThrow(`Multiple maps with the same id: ${uniqueId}`)
})
