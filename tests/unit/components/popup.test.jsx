...
describe('Popup Component', () => {
  ...
  test('renders a popup with correct props', () => {
    ...
    expect(mockMapLib.Popup).toHaveBeenCalledWith(
  ...
  test('registers event handlers correctly', () => {
    ...
    render(
      <MapContext.Provider value={mapContextValue}>
        <Popup longitude={10} latitude={20} onOpen={onOpen} onClose={onClose}>
          <div>Popup content</div>
        </Popup>
      </MapContext.Provider>
    )
  ...
  test('updates popup properties when props change', () => {
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Popup longitude={10} latitude={20} offset={10} maxWidth='200px' className='popup-class'>
          <div>Popup content</div>
        </Popup>
      </MapContext.Provider>
    )
  ...
  test('applies custom style to popup element', () => {
    ...
    render(
      <MapContext.Provider value={mapContextValue}>
        <Popup longitude={10} latitude={20} style={customStyle}>
    ...
    expect(applyReactStyle).toHaveBeenCalledWith(expect.any(HTMLElement), customStyle)
  })
