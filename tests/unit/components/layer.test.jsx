...
describe('Layer Component', () => {
  ...
  test('moves layer when beforeId changes', () => {
    ...
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' beforeId='layer-a' />
      </MapContext.Provider>
    )
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' beforeId='layer-b' />
      </MapContext.Provider>
    )

    // Should move the layer
    expect(mockMapInstance.moveLayer).toHaveBeenCalledWith('test-layer', 'layer-b')
  })
  ...
  test('updates filter property', () => {
    ...
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' filter={initialFilter} />
      </MapContext.Provider>
    )
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' filter={newFilter} />
      </MapContext.Provider>
    )
  ...
  test('updates minzoom and maxzoom properties', () => {
    ...
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' minzoom={5} maxzoom={15} />
      </MapContext.Provider>
    )
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' minzoom={2} maxzoom={10} />
      </MapContext.Provider>
    )
  ...
  test('throws error when layer type changes', () => {
    ...
    const { rerender } = render(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='test-source' />
      </MapContext.Provider>
    )
  ...
  test('throws error when layer id changes', () => {
    ...
    rerender(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='different-id' type='fill' source='test-source' />
      </MapContext.Provider>
    )
  ...
  test('handles case when source is not ready', () => {
    ...
    render(
      <MapContext.Provider value={mapContextValue}>
        <Layer id='test-layer' type='fill' source='not-ready-source' />
      </MapContext.Provider>
    )
  ...
  test('handles case when map is not available', () => {
    ...
    expect(() => {
      render(
        <MapContext.Provider value={mapContextValue}>
          <SafeLayer id='test-layer' type='fill' source='test-source' />
        </MapContext.Provider>
      )
    }).not.toThrow()
  })
  ...
  test('generates layer id if not provided', () => {
    render(
      <MapContext.Provider value={mapContextValue}>
        <Layer type='fill' source='test-source' />
      </MapContext.Provider>
    )
  ...
  test('updating a custom layer is a no-op (no layout/paint updates)', () => {
    const customLayer = {
      id: 'custom-layer',
      type: 'custom',
      onAdd: vi.fn(),
    ...
    // custom layers own their rendering — the wrapper must not touch them
  ...
  test('renders outside a Map component throw a descriptive error', () => {
    ...
    }).toThrow('<Layer> must be used within a Map component')
  })
})
