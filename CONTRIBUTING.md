...
Human walkthrough of every e2e case in one headed browser — a
bottom-center HUD pill per case, a live camera check on `map/ref-methods`
(zoom in/out + fly), and visible per-case demos (navigation zoom, globe
toggle — ends on globe, map click, marker drag, popup close/reopen,
fitBounds/pitch/bearing, layer hover, draw a polygon, video aspect/north-up
check, animation/orbit, useMap/useControl). Evidence prints to the console
and a report JSON under `tests/e2e/report/`. The review browser launches
with backgrounding/occlusion throttling disabled, so maplibre keeps painting
even when the window is covered — otherwise camera changes and demos apply
invisibly until you raise the window. Requires the e2e host app on :5175
(which serves the **built** `dist/` — rebuild after library changes, or you
review stale output). The runner fails fast with a message when the host app
isn't serving:

```bash
npm run e2e:serve                                  # terminal 1
npm run e2e:review                                 # terminal 2 — 10s dwell per case
npm run e2e:review -- --dwell=30000                # 30s per case
npm run e2e:review -- --dwell=0                    # no hold — fast evidence pass
npm run e2e:review -- --pause                      # wait for Enter per case
npm run e2e:review -- --only=map                   # one module or exact case id
```

The same knobs work as env vars (`DWELL`, `PAUSE`, `ONLY`) for parity with
`npm run test:framework:review`.
...
**CI vs local.** The Release workflow (`.github/workflows/release.yaml`)
runs on every push/PR: the gate (typecheck, lint, unit tests with coverage —
uploaded to [Codecov](https://codecov.io/gh/barikoi/react-bkoi-gl) — build,
README example check, pack smoke test) plus CodeQL security scanning (a
parallel job). On pushes to `master` it additionally regenerates the two
local badge SVGs and creates the GitHub Release. The browser-mode and e2e
suites (real maplibre, API key, built dist) stay local:
...
- README badges: coverage comes from Codecov, CI/CodeQL status from the
  GitHub Actions badges; bundle size and engine version are local SVGs
  regenerated by `npm run build` (`scripts/make-badges.mjs`).
- `package-lock.json` is committed (reproducible `npm ci` in CI);
  `package.json` version specifiers are exact pins, so it only pins
  transitives.
...
Publishing is **manual** — CI never publishes. The Release workflow only
refreshes the README badges from CI metrics and creates the GitHub Release
from the CHANGELOG.
...
1. Prepare the release on a branch:
   ...
   - Dry-run the notes extraction to preview release notes:
     ...
     ```
2. Merge into `master` — the Release workflow regenerates the two local
   badge SVGs (bundle size, engine version) and commits them back with
   `[skip ci]`, then creates the GitHub Release (below).
3. Publish from your machine (requires `npm login` + 2FA):
   ```bash
   npm publish --access public   # prepublishOnly gate runs automatically
   ```
4. The Release workflow tags `vX.Y.Z` and creates the GitHub Release with the
   CHANGELOG section as notes (`node scripts/extract-changelog.mjs` prints
   them). Verify it on the
   [releases page](https://github.com/barikoi/react-bkoi-gl/releases).
