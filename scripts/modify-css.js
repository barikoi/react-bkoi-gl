...
const applyCustomStyles = async cssFilePath => {
  try {
    ...
    const changes = {
      ...
      // Logo Control (Left-Bottom Corner)
...
const copyCssFile = async () => {
  ...
  // Source path for maplibre-gl-draw CSS
  const drawSrcPath = resolve('node_modules/maplibre-gl-draw/dist/mapbox-gl-draw.css')
  ...
  try {
    ...
    // Read maplibre-gl CSS
    let css = await fs.readFile(srcPath, 'utf8')

    // Read and append maplibre-gl-draw CSS
    try {
      const drawCss = await fs.readFile(drawSrcPath, 'utf8')
      css += '\n\n/* maplibre-gl-draw styles */\n' + drawCss
      console.log('Draw CSS appended successfully')
    } catch (drawError) {
      console.warn('Warning: Could not read maplibre-gl-draw CSS:', drawError.message)
    }
