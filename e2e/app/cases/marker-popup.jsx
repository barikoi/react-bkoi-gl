    ...
    <Section title="Markers — default, draggable, custom children">
      <TestMap section="marker-basic">
        ...
        <Marker data-testid="custom" longitude={90.4} latitude={23.83}>
          <div data-testid="custom-content" style={{ background: '#fff', padding: '5px 10px', borderRadius: 5 }}>
            Custom Marker
          </div>
        </Marker>
        <span data-testid="pos" style={{ display: 'none' }}>{`${pos.lng.toFixed(4)},${pos.lat.toFixed(4)}`}</span>
      </TestMap>
    </Section>
  )
}
